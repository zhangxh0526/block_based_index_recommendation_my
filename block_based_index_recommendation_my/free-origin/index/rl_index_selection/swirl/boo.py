...
class BagOfOperators(object):
    ...
    def _parse_bitmap_index_scan(self, node):
        node_representation = ""
        if "Relation Name" not in node:
            relation_name = node["Index Name"].split("_prt_p")[0]+"_prt_p"+node["Index Name"].split("_prt_p")[-1][0]
            node_representation +=f"{relation_name}_"
            node_representation += self._stringify_attribute_columns(node, "Index Cond")
        else:
            node_representation += f"{node['Relation Name']}_"
            node_representation += self._stringify_attribute_columns(node, "Filter")
    ...
    def _parse_dynamic_bitmap_index_scan(self, node):
        node_representation = ""
        if "Relation Name" not in node:
            relation_name = node["Index Name"].split("_")[0]+node["Index Name"].split("_")[1]
            node_representation +=f"{relation_name}_"
            node_representation += self._stringify_attribute_columns(node, "Index Cond")
        else:
            node_representation += f"{node['Relation Name']}_"
    ...
    def _parse_node(self, node):
        ...
        # 改动"Bitmap Heap Scan","Bitmap Index Scan",
        elif node["Node Type"] == "Bitmap Heap Scan":
            node_representation += f"{self._parse_seq_scan(node)}"
        elif node["Node Type"] == "Bitmap Index Scan":
            node_representation += f"{self._parse_bitmap_index_scan(node)}"
        elif node["Node Type"] == "Dynamic Seq Scan":
            node_representation += f"{self._parse_seq_scan(node)}"
        elif node["Node Type"] == "Dynamic Bitmap Index Scan":
            node_representation += f"{self._parse_dynamic_bitmap_index_scan(node)}"
        elif node["Node Type"] == "Dynamic Bitmap Heap Scan":
            node_representation += f"{self._parse_seq_scan(node)}"
        else:
