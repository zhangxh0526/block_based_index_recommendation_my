...
class ExtendAlgorithmPartitionHistogram(SelectionAlgorithm):
    ...
    def _partition_query_counts(self, workload):
        return [sum(q.frequency for q in workload.queries[i::self.partition_num]) for i in range(self.partition_num)]

    def _partition_sizes_mb(self, single_attribute_index_candidates):
        partition_tables = [set() for _ in range(self.partition_num)]
        for partition_id, candidate_list in enumerate(single_attribute_index_candidates):
        ...
        for tables in partition_tables:
            ...
            for table_name in tables:
                try:
                    size_bytes = self.database_connector.exec_fetch(
                        f"SELECT pg_relation_size('{table_name}')", one=True
                    )
                    size_bytes = size_bytes[0] if size_bytes else 0
                except Exception as exc:
                    logging.warning("pg_relation_size failed for %s: %s", table_name, exc)
                    size_bytes = 0
                total_bytes += size_bytes or 0
            partition_sizes_mb.append(b_to_mb(total_bytes))
        return partition_sizes_mb, partition_tables

    def _table_row_counts(self, partition_tables):
        row_counts = {}
        for tables in partition_tables:
            for table_name in tables:
                ...
                try:
                    ...
                    rows = 0
                row_counts[table_name] = max(int(rows), 0)
        return row_counts
    ...
    @staticmethod
    def _selectivity_factor(selectivity, cutoff=0.3, epsilon=1e-6):
        if selectivity is None:
            return 1.0
        if selectivity >= cutoff:
            return 0.0
        s = min(max(selectivity, epsilon), 1.0)
        return (1 - s) / (s + epsilon)

    def _partition_selectivity_factors(self, workload, partition_tables):
        factors = [1.0 for _ in range(self.partition_num)]
        row_counts = self._table_row_counts(partition_tables)
        logging.info("Partition row estimates: %s", row_counts)
        ...
        for partition_id in range(self.partition_num):
            ...
            for query in queries_partition:
                ...
                table_selectivities = []
                for table_name in partition_tables[partition_id]:
                    rows = row_counts.get(table_name, 0)
                    if rows <= 0:
                        continue
                    table_selectivities.append(plan_rows / max(rows, 1))

                if table_selectivities:
                    selectivity = min(table_selectivities)
                else:
                    selectivity = None

                factor = self._selectivity_factor(selectivity)
                weight = query.frequency
                total_weight += weight
                weighted_factor += factor * weight

            if total_weight > 0:
                factors[partition_id] = weighted_factor / total_weight

        logging.info("Partition selectivity factors: %s", factors)
        return factors

    def _compute_partition_budgets(self, single_attribute_index_candidates, workload):
        query_counts = self._partition_query_counts(workload)
        partition_sizes_mb, partition_tables = self._partition_sizes_mb(single_attribute_index_candidates)
        selectivity_factors = self._partition_selectivity_factors(workload, partition_tables)

        scores = []
        for count, size_mb, sel_factor in zip(query_counts, partition_sizes_mb, selectivity_factors):
            base = count * math.sqrt(max(size_mb, 0))
            scores.append(base * sel_factor)

        score_sum = sum(scores)
        if score_sum == 0:
            equal_weight = 1 / self.partition_num
            budgets_mb = [self.budget * equal_weight for _ in range(self.partition_num)]
            logging.info("Budget fallback to equal split (zero score)")
            return budgets_mb, query_counts, partition_sizes_mb, scores

        min_budget = float(self.parameters.get("min_partition_budget_MB", 5.0))

        weights = [score / score_sum for score in scores]
        base_budgets = [self.budget * weight for weight in weights]

        floor_indices = [
            i for i, (budget_mb, count, score) in enumerate(zip(base_budgets, query_counts, scores))
            if count > 0 and score > 0 and budget_mb < min_budget
        ]

        total_floor = len(floor_indices) * min_budget
        if total_floor > self.budget:
            active_partitions = sum(1 for count in query_counts if count > 0)
            active_partitions = active_partitions or 1
            even_budget = self.budget / active_partitions
            budgets_mb = [even_budget if count > 0 else 0 for count in query_counts]
            logging.warning(
                "Budget floor exceeds total budget; fallback to even split across active partitions"
            )
            return budgets_mb, query_counts, partition_sizes_mb, scores

        remaining_budget = self.budget - total_floor
        budgets_mb = [min_budget if i in floor_indices else 0 for i in range(self.partition_num)]

        for i in range(self.partition_num):
            budgets_mb[i] += remaining_budget * (scores[i] / score_sum)
    ...
    def _calculate_best_indexes(self, workload):
        ...
        workload_partition_budget, query_counts, partition_sizes_mb, scores = self._compute_partition_budgets(
            single_attribute_index_candidates,
            workload,
        )

        total_budget = sum(workload_partition_budget)
        self.partition_budget = [
            (budget / total_budget) if total_budget else 0 for budget in workload_partition_budget
        ]

        logging.info("Partition query counts: %s", query_counts)
        logging.info("Partition sizes (MB): %s", partition_sizes_mb)
        logging.info("Partition scores (count*sqrt(size)): %s", scores)
        logging.info("Partition budgets (MB): %s", workload_partition_budget)
        print(f"workload_partition_budget = {workload_partition_budget}")
        # 记录算法开始的时间戳
        ...
        for i in range(self.partition_num):
            ...
            print(f"workload_partition.budget = {workload_partition_budget[i]}")
            ...
            while True:
                ...
                single_attribute_index_candidates[i] = self._get_candidates_within_budget(
                    index_combination_size,
                    single_attribute_index_candidates[i],
                    workload_partition.budget,
                )
                # 遍历筛选后的单属性索引候选
    ...
    def _attach_to_indexes(self, index_combination, attribute, best, current_cost, budget_limit_mb, workload=None):
        """
    ...
    def _get_candidates_within_budget(self, index_combination_size, candidates, budget_limit_mb):
        """
        筛选出在预算范围内的索引候选。
        ...
        for candidate in candidates:
            ...
            if candidate.estimated_size + index_combination_size <= mb_to_b(budget_limit_mb):
    ...
    def _evaluate_combination(
        self, index_combination, best, current_cost, budget_limit_mb, old_index_size=0, workload=None
    ):
        """
        ...
        if ratio >= best["benefit_to_size_ratio"] and total_size <= mb_to_b(budget_limit_mb):
            # 记录日志，输出新的最佳成本和总大小
            logging.debug(
