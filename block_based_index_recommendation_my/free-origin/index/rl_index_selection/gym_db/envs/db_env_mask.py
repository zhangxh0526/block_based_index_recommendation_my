...
class DBEnvMask(gym.Env):
    def __init__(self, environment_type=EnvironmentType.TRAINING, config=None):
        ...
        self.histogram_manager = HistogramsManager(
            ...
            query_limit=config["workload_size"]
        )
        ...
        self.observation_space = self.observation_manager.get_histgramed_observation_space(self.histogram_manager)
    ...
    def step(self, action):
        ...
        start_time = time.time()
        current_observation = self.observation_manager.get_observation_histogramed(environment_state, self.histogram_manager)
        histgram_time = round(float(time.time() - start_time), 2)
        self.histgram_time_list.append(histgram_time)
        ...
        if episode_done and self.environment_type != EnvironmentType.TRAINING:
            # 计算总时间差
            total_seconds = sum(self.cost_time_list) / 4 + sum(self.histgram_time_list) / 10
            print(f"episode done current_workload.budget : {self.current_workload.budget}")
    ...
    def action_masks(self):
        # print("Action Mask Dim:{}".format([len(self.valid_total_actions), sum([action == 1 for action in self.valid_total_actions])]))
        # print("Total budget : {}, consumed budget : {}".format(self.current_budget, self.current_storage_consumption))
    ...
    def _calculate_final_cost_proportion(self, workload, indexes):
        ...
        return round(final_cost / (start_cost + 1) * 100, 2), start_cost, final_cost, start_rows, final_rows
