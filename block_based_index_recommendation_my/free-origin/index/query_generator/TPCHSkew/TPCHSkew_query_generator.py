...
if __name__ == "__main__":
    ...
    database_name = "indexselection_tpchskew___10"
    ...
    # verify max value
    max_shipdate = connector.exec_fetch(("select max(l_shipdate) from lineitem;"), False)[0][0]
    min_shipdate = connector.exec_fetch(("select min(l_shipdate) from lineitem;"), False)[0][0]

    max_commitdate = connector.exec_fetch(("select max(l_commitdate) from lineitem;"), False)[0][0]
    min_commitdate = connector.exec_fetch(("select min(l_commitdate) from lineitem;"), False)[0][0]

    max_receiptdate = connector.exec_fetch(("select max(l_receiptdate) from lineitem;"), False)[0][0]
    min_receiptdate = connector.exec_fetch(("select min(l_receiptdate) from lineitem;"), False)[0][0]

    print("Shipdate range: {}, {}\n Commitdate range: {}, {}\n Receiptdate range: {}, {}".format(min_shipdate, max_shipdate,
                                                                                                 min_commitdate, max_commitdate,
