...
from BISLearner.utils.utils import *
...
class BlockAwaredMultiColumnIndexActionManager(ActionManager):
    def __init__(
        ...
        self.number_of_columns = len(self.indexable_column_combinations[0])
        self.action_storage_consumptions = action_storage_consumptions
        self.number_of_index_per_partition = math.ceil(
            len(self.indexable_column_combinations_flat) / self.partition_num)
        ...
        block_index = [[]] * self.partition_num
        ...
        for partition_id in range(self.partition_num):
            ...
            for idx, cc in enumerate(block_index[partition_id]):
    ...
    def get_action_space(self):
        # 改动 return index_id
        return spaces.Discrete(self.number_of_index_per_partition)
    ...
    def update_valid_actions(self, last_action, budget, current_storage_consumption, block_id):
