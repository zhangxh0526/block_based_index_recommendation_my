...
from BISLearner.Env.utils.common import EnvironmentType
from BISLearner.Env.utils.cost_evaluation import CostEvaluation
from BISLearner.Env.utils.index import Index
from BISLearner.utils.utils import b_to_mb
...
class DBEnvV3(gym.Env):
    def __init__(self, environment_type=EnvironmentType.TRAINING, config=None):
        ...
        self.action_space = [gym.spaces.Discrete(self.partition_num), self.action_manager.get_action_space()]
    ...
    def _step_asserts(self, actions):
        assert self.action_space.contains(actions), f"{actions} ({type(actions)}) invalid"
        assert (
    ...
    def step(self, actions):
        ...
        self.valid_actions, is_valid_action_left = self.action_manager.update_valid_actions(
            index_action, self.current_budget, self.current_storage_consumption, block_id
        )
