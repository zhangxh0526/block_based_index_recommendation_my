...
class Experiment(object):
    def __init__(self, configuration_file):
        ...
        self.comparison_indexes = {
            "Extend": set(),
            "Extend_partition": set(),
            "DB2Adv": set(),
            "Slalom": set(),
        ...
        self.comparison_index_details = {
            ...
            "validation": {key: [] for key in self.comparison_indexes.keys()},
        }
    ...
    def compare(self):
        ...
        if "extend_global_independent" in self.config["comparison_algorithms"]:
    ...
    def _index_metadata(self, index):
        """
        将索引对象转换为包含基础元数据的字典，以便序列化到结果文件。
        """
        size_bytes = None
        if hasattr(self, "index_size_map"):
            size_bytes = self.index_size_map.get(index.columns)
        if size_bytes is None and getattr(index, "estimated_size", None) is not None:
            size_bytes = index.estimated_size

        size_mb = round(size_bytes / (1024 * 1024), 2) if size_bytes is not None else None

        return {
            "repr": str(index),
            "name": index.index_idx(),
    ...
    def _record_index_selection(self, run_type, algo_key, budget, indexes, final_cost_proportion, runtime_metrics=None):
        ...
        """
        index_details = [self._index_metadata(idx) for idx in indexes]
        total_size_bytes = sum(
            detail["size_bytes"] for detail in index_details if detail["size_bytes"] is not None
        )
    ...
    def _compare_extend_global_independent(self):
        """
        使用 Global Independent Extend（全局资金池、不区分分区）做对比实验。
        """
        self.evaluated_workloads = set()
        gi_connector = PostgresDatabaseConnector(self.schema.database_name, autocommit=True)
        gi_connector.drop_indexes()
        gi_algorithm = ExtendAlgorithmGlobalIndependent(gi_connector)

        run_type = "test"
        for test_wl in self.workload_generator.wl_testing[0]:
            self.comparison_performances[run_type]["Extend_global_independent"].append([])
            ...
            gi_algorithm.reset(parameters)
            indexes = gi_algorithm.calculate_best_indexes(test_wl)
            self.comparison_indexes["Extend_global_independent"] |= frozenset(indexes)
            self._record_index_selection(
                run_type,
                "Extend_global_independent",
                test_wl.budget,
    ...
    def _compare_extend_global(self):
        """
        使用 True Global Greedy Extend（单一全局预算，不分区）做对比实验。
        """
        self.evaluated_workloads = set()
        gg_connector = PostgresDatabaseConnector(self.schema.database_name, autocommit=True)
        gg_connector.drop_indexes()
        gg_algorithm = ExtendAlgorithmGlobal(gg_connector)

        run_type = "test"
        for test_wl in self.workload_generator.wl_testing[0]:
            self.comparison_performances[run_type]["Extend_global"].append([])
            ...
            gg_algorithm.reset(parameters)
            indexes = gg_algorithm.calculate_best_indexes(test_wl)
            self.comparison_indexes["Extend_global"] |= frozenset(indexes)
            self._record_index_selection(
                run_type,
                "Extend_global",
                test_wl.budget,
        ...
        for validation_wl in self.workload_generator.wl_validation[0]:
            ...
            gg_algorithm.reset(parameters)
            indexes = gg_algorithm.calculate_best_indexes(validation_wl)
            self.comparison_indexes["Extend_global"] |= frozenset(indexes)
            self._record_index_selection(
                run_type,
                "Extend_global",
    ...
    def _compare_slalom(self):
        ...
        for test_wl in self.workload_generator.wl_testing[0]:
            ...
            self._record_index_selection(
                run_type, "Slalom", test_wl.budget, indexes, slalom_algorithm.final_cost_proportion
            )
        ...
        for validation_wl in self.workload_generator.wl_validation[0]:
            ...
            self._record_index_selection(
                ...
                validation_wl.budget,
                indexes,
                slalom_algorithm.final_cost_proportion,
            )
            self.comparison_performances[run_type]["Slalom"][-1].append(slalom_algorithm.final_cost_proportion)
