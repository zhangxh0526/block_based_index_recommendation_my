...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            columns = random.sample(_columns, 1)
            columns.sort()
            _col_i = 0
            for col in columns:
                if "date" in col:
            ...
            query = query_template.format(x0=_ranges[0], x1=_ranges[1], x2=_ranges[2])
            ...
            for _ in bound:
                if _[1] <= tmp_count <= _[2]:
                    ...
                    with open(query_path, "a+") as query_file:
                        ...
                        query_file.write(
                            query.replace("count(*)", "select l_returnflag, l_linestatus, sum(l_quantity) as sum_qty, sum(l_extendedprice) as sum_base_price, sum(l_extendedprice * (1 - l_discount)) as sum_disc_price, sum(l_extendedprice * (1 - l_discount) * (1 + l_tax)) as sum_charge, avg(l_quantity) as avg_qty, avg(l_extendedprice) as avg_price, avg(l_discount) as avg_disc, count(*) as count_order") + "group by l_returnflag, l_linestatus order by l_returnflag, l_linestatus ;\n")
                        print(
                            f"Found parameters (x0={_ranges[0]}, x1={_ranges[1]}, x2={_ranges[2]}) with tmp_count: {tmp_count}")
