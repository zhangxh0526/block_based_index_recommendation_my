...
def draw_training_time():
    ...
    print("Faster than Swirl: {}".format([Swirl[_] / BISLearner[_] for _ in range(len(BISLearner))]))
    # Swirl = [_*random.uniform(1.3, 1.7) for _ in BISLearner]
    # Swirl[2] = training_time_from_file("Swirl_tpch2.log")
    ...
    plt.bar(x_values[0], Swirl, alpha=0.9, width=bar_width, label='Swirl (using BIS Act)', color='#76da91')
    plt.bar(x_values[1], BISLearner, alpha=0.9, width=bar_width, label='BISLearner', color='#ff7f0e')
